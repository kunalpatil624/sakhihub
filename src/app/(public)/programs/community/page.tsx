...
import { Users, Heart, Share2, ShieldCheck, Globe, CheckCircle2 } from 'lucide-react';
    ...
    <main className="overflow-x-hidden">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 lg:gap-20 items-center">
             ...
             >
               <h2 className="text-3xl sm:text-4xl lg:text-5xl font-bold text-secondary mb-6 leading-tight">
                 {t('communityPage.unityTitle').split(' ').slice(0, -1).join(' ')} <span className="text-gradient">{t('communityPage.unityTitle').split(' ').slice(-1).join(' ')}</span>
               </h2>
               <p className="text-base sm:text-lg lg:text-xl text-gray-600 leading-relaxed mb-10 font-medium">
                 {t('communityPage.unityDesc')}
               </p>

               <div className="grid gap-6 mb-10 text-left max-w-xl mx-auto lg:mx-0">
                  {[
                    { title: t('communityPage.bullet1Title'), desc: t('communityPage.bullet1Desc') },
                    { title: t('communityPage.bullet2Title'), desc: t('communityPage.bullet2Desc') },
                    { title: t('communityPage.bullet3Title'), desc: t('communityPage.bullet3Desc') }
                  ].map((item, i) => (
      ...
      <section className="section-padding bg-gray-50">
        <div className="container">
           <div className="text-center mb-12 lg:mb-16">
              <h2 className="text-3xl sm:text-4xl lg:text-5xl font-bold text-secondary leading-tight">{t('communityPage.brotherhood').split(' ').slice(0, -1).join(' ')} <span className="text-gradient">{t('communityPage.brotherhood').split(' ').slice(-1).join(' ')}</span></h2>
           </div>

           <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 lg:gap-8">
              {[
                { label: t('communityPage.villageGroups'), val: '500+', icon: Users },
                { label: t('communityPage.activeSakhis'), val: '50k+', icon: Heart },
                { label: t('communityPage.successStories'), val: '1,200+', icon: ShieldCheck },
                { label: t('communityPage.totalReach'), val: '100k+', icon: Globe },
              ].map((item, i) => (
      ...
      <section className="section-padding bg-secondary text-white text-center relative overflow-hidden">
        <div className="container relative z-10">
           <h2 className="text-3xl sm:text-4xl lg:text-5xl font-bold mb-8 leading-tight">{t('communityPage.walkAlone').split(' ').slice(0, -1).join(' ')} <span className="text-primary">{t('communityPage.walkAlone').split(' ').slice(-1).join(' ')}</span></h2>
           <p className="text-base sm:text-lg lg:text-xl opacity-80 max-w-2xl mx-auto mb-12 lg:mb-16 font-medium leading-relaxed">
             {t('communityPage.walkAloneDesc')}
           </p>
