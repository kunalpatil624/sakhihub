      ...
      <div className="flex flex-col gap-8">
        <header className="flex justify-between items-start flex-wrap gap-6 mb-4">
          ...
          <div className="flex gap-4">
             <div className="bg-white p-4 rounded-3xl border border-gray-100 shadow-soft flex items-center gap-3">
                <div className="w-10 h-10 rounded-2xl bg-secondary/10 flex items-center justify-center text-secondary"><Target size={20} /></div>
                <div>
        ...
        {loading ? (
          <div className="bg-white p-20 rounded-[40px] border border-gray-100 shadow-soft text-center">
            <div className="text-gray-400 font-bold italic animate-pulse">Syncing campaigns...</div>
          </div>
        ) : (
          <div className="flex flex-col gap-10">
            ...
            {assigned.length === 0 && requested.length === 0 && available.length === 0 && (
              <div className="bg-white p-20 rounded-[40px] border-2 border-dashed border-gray-100 text-center">
                 ...
                 <p className="text-gray-400 font-bold mt-2">Wait for the admin or vendor to launch new drives.</p>
              </div>
            )}
          </div>
        )}
...
function CampaignCard({ camp, type, onFetch }: { camp: any, type: string, onFetch?: () => void }) {
  ...
  const handleRequest = async () => {
    setRequesting(true);
    try {
      const res = await axios.post('/api/sub-vendor/campaigns', { campaignId: camp._id });
      if (res.data.success) {
        if (onFetch) onFetch();
      } else {
    ...
    >
      ...
      <div className="flex-1">
        ...
        <div className="flex flex-wrap gap-4 mt-6">
           ...
           {/* Charges hidden for sub-vendors */}
        </div>
      </div>

      <div className="flex flex-wrap lg:flex-nowrap gap-4 shrink-0 mt-4 lg:mt-0">
         {type === 'assigned' && (
           <button className="w-full flex items-center justify-center gap-3 px-8 py-4 rounded-2xl bg-secondary text-white font-black text-xs uppercase tracking-widest shadow-xl shadow-secondary/20 hover:scale-[1.02] active:scale-95 transition-all">
             Track Performance <ArrowRight size={18} />
           </button>
         )}
